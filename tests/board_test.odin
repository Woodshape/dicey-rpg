...
full_board_perimeter_is_outer_ring :: proc(t: ^testing.T) {
	...
	testing.expect(t, !game.cell_is_pickable(&board, centre, centre), "centre should not be perimeter on full board")
}
...
board_gradient_outer_ring_has_no_d10_d12 :: proc(t: ^testing.T) {
	...
	for _ in 0 ..< SAMPLE_BOARDS {
		...
		testing.expectf(t, d10_count == 0 && d12_count == 0,
			"outer ring should have no d10/d12, got d10=%d d12=%d", d10_count, d12_count)
	}
}
...
count_die_type_in_ring :: proc(board: ^game.Board, ring: int, target: game.Die_Type) -> int {
	...
	for row in 0 ..< board.size {
		for col in 0 ..< board.size {
			if game.cell_ring(board, row, col) == ring && board.cells[row][col].die_type == target {
				count += 1
			}
		}
	}
	...
	for _ in 0 ..< SAMPLE_BOARDS {
		...
		for row in 0 ..< board.size {
			for col in 0 ..< board.size {
				...
				case .D6:    counts.d6 += 1
				case .D8:    counts.d8 += 1
				case .D10:   counts.d10 += 1
				case .D12:   counts.d12 += 1
				case .Skull: counts.skull += 1
				}
			}
		}
	}
...
gradient_outer_ring_is_mostly_small_dice :: proc(t: ^testing.T) {
	...
	testing.expectf(t, c.d10 == 0 && c.d12 == 0,
		"outer ring should have no d10/d12, got d10=%d d12=%d", c.d10, c.d12)
	...
	if non_skull > 0 {
		...
		testing.expectf(t, d8_ratio < 0.25,
			"outer ring d8 should be <25%% of non-skull dice, got %.1f%%", d8_ratio * 100)
	}
}
...
gradient_inner_ring_has_d10_and_d12 :: proc(t: ^testing.T) {
	...
	testing.expectf(t, c.d10 > 0, "inner ring should have d10 (got 0 across %d boards)", SAMPLE_BOARDS)
	testing.expectf(t, c.d12 > 0, "inner ring should have d12 (got 0 across %d boards)", SAMPLE_BOARDS)
}
...
gradient_d8_peaks_at_middle_depth :: proc(t: ^testing.T) {
	...
	testing.expectf(t, c.d8 > 0, "middle ring (ring %d) should have d8 (got 0 across %d boards)", mid_ring, SAMPLE_BOARDS)
}
...
gradient_monotonic_high_tier_increases_with_depth :: proc(t: ^testing.T) {
	...
	for ring in 0 ..< max_ring {  // exclude centre (forced d12)
		...
		testing.expectf(t, ratio >= prev_ratio,
			"ring %d d8+ ratio (%.2f) should be >= previous ring (%.2f)",
			ring, ratio, prev_ratio)
		prev_ratio = ratio
	}
}
...
gradient_all_five_die_types_appear_on_board :: proc(t: ^testing.T) {
	...
	for _ in 0 ..< SAMPLE_BOARDS {
		...
		for row in 0 ..< board.size {
			for col in 0 ..< board.size {
				#partial switch board.cells[row][col].die_type {
				case .D4:  found_d4 = true
				case .D6:  found_d6 = true
				case .D8:  found_d8 = true
				case .D10: found_d10 = true
				case .D12: found_d12 = true
				}
			}
		}
	...
	testing.expectf(t, found_d4,  "d4 should appear on at least one board across %d samples", SAMPLE_BOARDS)
	testing.expectf(t, found_d6,  "d6 should appear on at least one board across %d samples", SAMPLE_BOARDS)
	testing.expectf(t, found_d8,  "d8 should appear on at least one board across %d samples", SAMPLE_BOARDS)
	testing.expectf(t, found_d10, "d10 should appear on at least one board across %d samples", SAMPLE_BOARDS)
	testing.expectf(t, found_d12, "d12 should appear on at least one board across %d samples", SAMPLE_BOARDS)
}
...
gradient_skull_dice_appear_in_all_rings :: proc(t: ^testing.T) {
	...
	for ring in 0 ..< max_ring {  // exclude centre (forced d12, no skulls)
		c := count_ring_distribution(ring)
		testing.expectf(t, c.skull > 0,
			"ring %d should have skull dice across %d boards, got 0", ring, SAMPLE_BOARDS)
	}
}
